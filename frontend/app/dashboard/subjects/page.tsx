...
import { fetchSubjectsList } from "@/lib/subjects-server";
import type { Metadata } from "next";
import Link from "next/link";

export const metadata: Metadata = {
  title: "Subjects — All Citizens",
};

function listFetchHint(
  result: Awaited<ReturnType<typeof fetchSubjectsList>>,
): string | null {
  ...
  switch (result.reason) {
    ...
    case "no_tenant":
      return "Set TENANT_ID in the frontend .env to list subjects for the tenant in the API.";
    case "upstream":
      return `Could not load subjects from the API (HTTP ${result.status ?? "—"}). Check that the backend is running and the token is authorized.`;
    default:
      return null;
  }
}
...
export default async function SubjectsPage() {
  const result = await fetchSubjectsList(0, 50);
  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs font-semibold text-gray-500">
            ...
            <span className="text-on-surface">Subjects</span>
          </div>
          <h1 className="text-2xl font-bold tracking-tight text-on-surface">
            Subjects
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            Catalog grouping for services by tenant.
          </p>
        </div>
        <Link
      ...
      <SubjectsTable rows={rows} total={total} fetchHint={fetchHint} />
    </>
  );
}
